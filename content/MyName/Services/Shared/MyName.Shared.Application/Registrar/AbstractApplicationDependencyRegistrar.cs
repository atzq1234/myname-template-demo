using MyName.Shared.Application.Services.Trackers;
...
public abstract partial class AbstractApplicationDependencyRegistrar : IDependencyRegistrar
{
    ...
    /// 注册所有服务
    /// </summary>
    public abstract void AddMyName();

    /// <summary>
    /// 注册MyName.application通用服务
    /// </summary>
    protected virtual void AddApplicaitonDefault()
    {
        Services
            .AddValidatorsFromAssembly(ContractsLayerAssembly, ServiceLifetime.Scoped)
            .AddMyNameInfraAutoMapper(ApplicationLayerAssembly)
            .AddMyNameInfraYitterIdGenerater(RedisSection)
            .AddMyNameInfraConsul(ConsulSection);
