namespace MyName.Infra.Repository.EfCore.Transaction;

public abstract class UnitOfWork<TDbContext> : IUnitOfWork
    where TDbContext : DbContext
{
    protected TDbContext MyNameDbContext { get; init; }
    protected IDbContextTransaction? DbTransaction { get; set; }

    public bool IsStartingUow => MyNameDbContext.Database.CurrentTransaction is not null;

    protected UnitOfWork(TDbContext context)
    {
        MyNameDbContext = context ?? throw new ArgumentNullException(nameof(context));
    }
    ...
    {
        if (MyNameDbContext.Database.CurrentTransaction is not null)
            throw new ArgumentException($"UnitOfWork Error,{MyNameDbContext.Database.CurrentTransaction}");
        else
